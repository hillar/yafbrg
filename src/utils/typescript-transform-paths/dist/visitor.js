...
function nodeVisitor(node) {
    ...
    if (tsInstance.isImportTypeNode(node)) {
        ...
        if (!text)
            return node;
        return (0, utils_1.resolvePathAndUpdateNode)(this, node, text, (p) => factory.updateImportTypeNode(node, factory.updateLiteralTypeNode(argument, p), node.qualifier, node.typeArguments, node.isTypeOf));
    }
    /**
    ...
    if (tsInstance.isModuleDeclaration(node) && tsInstance.isStringLiteral(node.name))
        return (0, utils_1.resolvePathAndUpdateNode)(this, node, node.name.text, (p) => factory.updateModuleDeclaration(node, node.decorators, node.modifiers, p, node.body));
